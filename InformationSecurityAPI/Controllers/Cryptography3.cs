...
namespace InformationSecurityAPI.Controllers
{
    ...
    public class Cryptography3 : Controller
    {
        ...
        public BigInteger GenerateNumberTestSoloveyStrassen(int bitCount)
        {
            while (true)
            {
        ...
        public BigInteger GenerateNumberTestPocklington(int bitCount)
        {
            while (true)
            {
                var rng = new RNGCryptoServiceProvider();
                ...
                if (shifrovanie5.TestPocklington(p) == "Вероятно простое")
                {
                    return p;
                }
            }
        ...
        {
            BigInteger a = GenerateNumberTestSoloveyStrassen(64);
            BigInteger g = GenerateNumberTestPocklington(64);
            BigInteger p = GenerateNumberTestSoloveyStrassen(512);
        ...
        {
            BigInteger b = GenerateNumberTestSoloveyStrassen(64);
        ...
        [HttpPost]
        [Route("[action]")]
        public IActionResult GetPogrlinton(CryptographyModel3 res)
        {
            // string pogklin = shifrovanie5.TestPocklington(BigInteger.Parse(res.A));
            BigInteger g = GenerateNumberTestPocklington(64);
            var response = new { K = g.ToString() };
            return Json(response);
        }
    }
}